...
QComboBox::down-arrow { image: url(:/omegacat/icons/chevron-down.svg); width: 10px; height: 10px; }
...
QCheckBox::indicator:checked {
    background: %accent%; border-color: %accent%; image: url(%checkImage%);
}
...
QListWidget::indicator:checked, QTreeWidget::indicator:checked, QTableView::indicator:checked {
    background: %accent%; border-color: %accent%; image: url(%checkImage%);
}
...
QTableView, QTreeWidget, QListWidget {
    background: %bgInput%; border: 1px solid %borderDim%; border-radius: 6px;
    color: %textPrimary%; gridline-color: %borderDim%; alternate-background-color: %bgSecondary%;
    selection-background-color: %bgSelected%; selection-color: %textPrimary%;
    outline: none;
}
QTreeWidget::item, QListWidget::item { padding: 3px 4px; }
QTreeWidget::item:hover, QListWidget::item:hover { background: %bgHover%; }
QTreeWidget::item:selected, QListWidget::item:selected { background: %bgSelected%; color: %accent%; }
QFrame[role="labMatch"] { border-radius: 8px; border: 1px solid %borderSecondary%; background: %bgTertiary%; }
QFrame[role="labMatch"][state="ok"] { background: %chipState%; border-color: %success%; }
QFrame[role="labMatch"][state="fail"] { background: %chipDanger%; border-color: %danger%; }
QFrame[role="labMatch"][state="fallback"] { background: %chipWarn%; border-color: %warning%; }
QFrame[role="labMatch"] QLabel { background: transparent; }
QFrame[role="labMatch"][state="ok"] QLabel[role="labCaption"] { color: %success%; }
QFrame[role="labMatch"][state="fail"] QLabel[role="labCaption"] { color: %danger%; }
QFrame[role="labMatch"][state="fallback"] QLabel[role="labCaption"] { color: %warning%; }
QTextBrowser[role="notices"] {
    background: %bgInput%; border: 1px solid %borderDim%; border-radius: 6px;
    color: %textPrimary%; padding: 6px;
}
QPlainTextEdit[role="content"] {
    background: %bgInput%; border: 1px solid %borderDim%; border-radius: 6px;
    color: %textPrimary%; padding: 6px;
}
QPushButton[role="viewTab"] {
    border: none; border-bottom: 2px solid transparent; border-radius: 0;
    padding: 8px 14px; color: %textSecondary%; font-weight: 600;
}
QPushButton[role="viewTab"]:hover { color: %accent%; }
QPushButton[role="viewTab"]:checked { color: %accent%; border-bottom: 2px solid %accent%; background: transparent; }
QProgressBar {
    background: %bgTertiary%; border: 1px solid %borderDim%; border-radius: 5px;
...
/* ---- dialogs from Omega (credential manager, editor, vault unlock) ----
   Roles and properties those dialogs set, in this theme's colours. */
QWidget[bare="true"] { background: transparent; }
QDialog[chromeless="true"] { background: %bgPrimary%; border: 1px solid %borderDim%; }
QFrame[role="chrome"] { background: %bgSecondary%; border-bottom: 1px solid %borderDim%; }
QFrame[role="well"] { background: %bgInput%; border: 1px solid %borderSecondary%; border-radius: 8px; }
QFrame[role="bar"] { background: %bgSecondary%; border-top: 1px solid %borderDim%; }
QFrame[role="noticebox"] { background: %noticeBg%; border: 1px solid %borderSecondary%; border-radius: 9px; }
QLabel[role="fieldlabel"], QLabel[role="desc"] { color: %textSecondary%; }
QLabel[role="notice"] { color: %textPrimary%; background: transparent; border: none; }
QLabel[chip="true"] { background: %bgTertiary%; border-radius: 5px; padding: 3px 8px;
    font-family: "%monoFamily%"; color: %textSecondary%; }
QLabel[chip="accent"] { background: %chipAccent%; border-radius: 5px; padding: 3px 8px;
    font-family: "%monoFamily%"; color: %accent%; }
QLabel[chip="state"] { background: %chipState%; border-radius: 5px; padding: 3px 8px;
    font-family: "%monoFamily%"; color: %success%; }
QLabel[chip="warn"] { background: %chipWarn%; border-radius: 5px; padding: 3px 8px;
    font-family: "%monoFamily%"; color: %warning%; }
QLabel[mono="true"] { font-family: "%monoFamily%"; color: %textSecondary%; }
QLabel[mono="true"][tone="ink"] { color: %textSecondary%; }
QLabel[mono="true"][tone="state"], QLabel[tone="state"] { color: %success%; }
QLabel[mono="true"][tone="warn"], QLabel[tone="warn"] { color: %warning%; }
QLabel[mono="true"][tone="hint"] { color: %textMuted%; }
QLineEdit[mono="true"], QPlainTextEdit[mono="true"] { font-family: "%monoFamily%"; }
QPushButton[destructive="true"] { background: %bgTertiary%; border: 1px solid %danger%; color: %danger%; }
QPushButton[destructive="true"]:hover { background: %danger%; color: %onAccent%; border-color: %danger%; }
QPushButton[destructive="true"]:disabled { color: %textMuted%; border-color: %borderDim%; background: transparent; }
QGroupBox { color: %textPrimary%; border: 1px solid %borderDim%; border-radius: 8px;
    margin-top: 10px; padding-top: 10px; background: transparent; }
QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; padding: 0 6px; color: %textSecondary%; }
QTabWidget::pane { background: %bgPrimary%; border: none; border-top: 1px solid %borderDim%; }
QTabBar { background: transparent; qproperty-drawBase: 0; }
QTabBar::tab { background: transparent; color: %textSecondary%; padding: 8px 12px;
    border-top-left-radius: 7px; border-top-right-radius: 7px; }
QTabBar::tab:hover:!selected { background: %bgHover%; color: %textPrimary%; }
QTabBar::tab:selected { background: %bgPrimary%; color: %textPrimary%;
    border: 1px solid %borderDim%; border-bottom-color: %bgPrimary%; }
QDialogButtonBox { button-layout: 3; }
QMessageBox { background: %bgSecondary%; }
...
QScrollBar::add-page, QScrollBar::sub-page { background: none; }
    ...
    // Tinted surfaces for chips and notices: the tone over the panel colour.
    const auto blend = [](const QColor &tone, const QColor &base, double a) {
        return QColor(int(base.red() + (tone.red() - base.red()) * a),
                      int(base.green() + (tone.green() - base.green()) * a),
                      int(base.blue() + (tone.blue() - base.blue()) * a));
    };
    s.replace(QLatin1String("%chipAccent%"), hex(blend(t.accent, t.bgSecondary, 0.16)));
    s.replace(QLatin1String("%chipState%"), hex(blend(t.success, t.bgSecondary, 0.16)));
    s.replace(QLatin1String("%chipWarn%"), hex(blend(t.warning, t.bgSecondary, 0.16)));
    s.replace(QLatin1String("%chipDanger%"), hex(blend(t.danger, t.bgSecondary, 0.14)));
    s.replace(QLatin1String("%noticeBg%"), hex(blend(t.warning, t.bgSecondary, 0.08)));
    s.replace(QLatin1String("%monoFamily%"), QFontDatabase::systemFont(QFontDatabase::FixedFont).family());
    // The check mark is drawn in the colour that reads on the accent fill.
    s.replace(QLatin1String("%checkImage%"),
              t.dark ? QStringLiteral(":/omegacat/icons/check-dark.svg") : QStringLiteral(":/omegacat/icons/check.svg"));
    return s;
}
...
// The palette as well as the sheet. A widget with autoFillBackground paints
// the palette's Window colour and never consults the stylesheet: a scroll
// area's content widget is one (QScrollArea::setWidget turns it on), and so
// the Omega dialogs' bodies stayed Fusion's light grey on every theme. Views
// that only use the sheet are unaffected; everything that falls through to the
// palette now falls through to this theme.
static QPalette paletteFor(const Tokens &t) {
    QPalette p;
    const QColor onAccent = t.dark ? t.bgPrimary : QColor(Qt::white);
    for (QPalette::ColorGroup g : {QPalette::Active, QPalette::Inactive, QPalette::Disabled}) {
        const bool off = g == QPalette::Disabled;
        p.setColor(g, QPalette::Window, t.bgPrimary);
        p.setColor(g, QPalette::WindowText, off ? t.textMuted : t.textPrimary);
        p.setColor(g, QPalette::Base, t.bgInput);
        p.setColor(g, QPalette::AlternateBase, t.bgSecondary);
        p.setColor(g, QPalette::Text, off ? t.textMuted : t.textPrimary);
        p.setColor(g, QPalette::PlaceholderText, t.textMuted);
        p.setColor(g, QPalette::Button, t.bgTertiary);
        p.setColor(g, QPalette::ButtonText, off ? t.textMuted : t.textSecondary);
        p.setColor(g, QPalette::Highlight, t.accent);
        p.setColor(g, QPalette::HighlightedText, onAccent);
        p.setColor(g, QPalette::ToolTipBase, t.bgTertiary);
        p.setColor(g, QPalette::ToolTipText, t.textPrimary);
        p.setColor(g, QPalette::Link, t.accent);
        p.setColor(g, QPalette::Mid, t.borderSecondary);
        p.setColor(g, QPalette::Dark, t.borderSecondary);
        p.setColor(g, QPalette::Light, t.bgSecondary);
    }
    return p;
}

void ThemeManager::setTheme(ThemeId id) {
    initResources();  // the sheet refers to :/omegacat images
    m_id = id;
    if (qApp) {
        QApplication::setPalette(paletteFor(tokensFor(id)));
        qApp->setStyleSheet(styleSheet(tokensFor(id)));
    }
    emit changed();
}
