...
using omegacat::AuthMethod;
using omegacat::CredentialInput;
using omegacat::CredentialMeta;
...
void CredentialEditorDialog::buildUi() {
    ...
    for (const AuthMethod m : {AuthMethod::Password, AuthMethod::PublicKey,
        ...
        auth_->addItem(omegacat::authLabel(m), static_cast<int>(m));
    }
    ...
    const int at = auth_->findData(
        static_cast<int>(omegacat::authMethodFromLabel(existing_.authLabel)));
...
void CredentialEditorDialog::showEvent(QShowEvent *event) {
    ...
    omegacat::alignFieldLabels(tabs_);
    if (frame_) frame_->fitToContent();
}
