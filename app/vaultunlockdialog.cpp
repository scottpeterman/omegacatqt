...
using omegacat::Vault;
using omegacat::VaultError;

namespace {
constexpr int kDialogWidth = 580;
}  // namespace

VaultUnlockDialog::VaultUnlockDialog(omegacat::Vault *vault, QWidget *parent)
    ...
    if (creating_) {
        frame_->addNotice(
            tr("There is no recovery. This password is the only thing that "
               "opens the vault, and nothing in OmegaCat or on this machine can "
               "reconstruct it."));
    }
...
void VaultUnlockDialog::setQuietResult(omegacat::VaultError code) {
    switch (code) {
