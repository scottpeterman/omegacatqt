...
// omegacat_free. Taking it into a QString the moment it arrives means there is
...
Vault::Vault(const QString &path) : path_(path) {
    handle_ = omegacat_vault_open(path.toUtf8().constData());
    if (handle_ <= 0) {
        lastError_ = takeString(omegacat_last_error());
    }
}

Vault::~Vault() {
    if (handle_ > 0) omegacat_vault_close(handle_);
}
...
VaultError Vault::finish(int code) const {
    ...
    lastError_ = (e == VaultError::Ok) ? QString()
                                       : takeString(omegacat_last_error());
    return e;
}

bool Vault::exists() const {
    return handle_ > 0 && omegacat_vault_exists(handle_) == 1;
}

bool Vault::isLocked() const {
    // A bad handle reports locked, which is the safe answer and the one the C
    // surface already gives.
    return handle_ <= 0 || omegacat_vault_is_locked(handle_) == 1;
}

VaultError Vault::create(const QString &master) {
    return finish(omegacat_vault_create(handle_, master.toUtf8().constData()));
}

VaultError Vault::unlock(const QString &master) {
    return finish(omegacat_vault_unlock(handle_, master.toUtf8().constData()));
}

VaultError Vault::unlockQuiet() {
    return finish(omegacat_vault_unlock_quiet(handle_));
}

void Vault::lock() {
    if (handle_ > 0) omegacat_vault_lock(handle_);
}

VaultError Vault::changeMaster(const QString &oldMaster, const QString &newMaster) {
    return finish(omegacat_vault_change_master(handle_,
...
VaultError Vault::list(QVector<CredentialMeta> *out) const {
    char *json = nullptr;
    const VaultError e = finish(omegacat_vault_list(handle_, &json));
...
VaultError Vault::meta(const QString &idOrName, CredentialMeta *out) const {
    char *json = nullptr;
    const VaultError e =
        finish(omegacat_vault_meta(handle_, idOrName.toUtf8().constData(), &json));
    const QString text = takeString(json);
...
VaultError Vault::store(const CredentialInput &input, QString *idOut) {
    ...
    char *id = nullptr;
    const VaultError e = finish(omegacat_vault_store(handle_, payload.constData(), &id));
...
VaultError Vault::updateMetadata(const CredentialMeta &m) {
    ...
    return finish(omegacat_vault_update_meta(handle_, payload.constData()));
}

VaultError Vault::remove(const QString &id) {
    return finish(omegacat_vault_delete(handle_, id.toUtf8().constData()));
}

VaultError Vault::rename(const QString &id, const QString &newName) {
    return finish(omegacat_vault_rename(handle_, id.toUtf8().constData(),
                                        newName.trimmed().toUtf8().constData()));
}

VaultError Vault::setDefault(const QString &id) {
    return finish(omegacat_vault_set_default(handle_, id.toUtf8().constData()));
}

VaultError Vault::clearDefault() {
    return finish(omegacat_vault_clear_default(handle_));
}

VaultError Vault::defaultName(QString *out) const {
    char *name = nullptr;
    const VaultError e = finish(omegacat_vault_default_name(handle_, &name));
...
VaultError Vault::setDisabled(const QString &id, bool disabled) {
    return finish(
        omegacat_vault_set_disabled(handle_, id.toUtf8().constData(), disabled ? 1 : 0));
}

VaultError Vault::keyringStatus(KeyringStatus *out) const {
    char *json = nullptr;
    const VaultError e = finish(omegacat_vault_keyring_status(handle_, &json));
    const QString text = takeString(json);
    if (e != VaultError::Ok || !out) return e;
...
VaultError Vault::keyringSet(const QString &master) {
    return finish(omegacat_vault_keyring_set(handle_, master.toUtf8().constData()));
}

VaultError Vault::keyringClear() {
    return finish(omegacat_vault_keyring_clear(handle_));
}

QString Vault::errorName(VaultError e) {
    // Owned by the library and not freed -- the one exception to the
    // omegacat_free rule, because it is a constant.
    return QString::fromUtf8(
        omegacat_vault_error_name(static_cast<omegacat_vault_err>(e)));
}
