// app/vault.h
//
// C++ front end for the vault C surface (include/omegacat/vault.h), ported
// from omegamaps, and before that from Omega's qt/omegasshvault.h with SNMP added. Same house rules: QtCore
// types only, no QtWidgets, one owner for every allocation, and nothing that
// hands a secret back.
//
// THE SHAPE IS INHERITED, NOT INVENTED. include/omegacat/vault.h says it in
...
// them closes it out from under the other. A capture takes handle(), and the
// Vault must outlive every capture started with it -- in practice the window
// owns one Vault for the life of the process.

#ifndef OMEGACAT_APP_VAULT_H
#define OMEGACAT_APP_VAULT_H
...
// Mirrors omegacat_vault_err value for value. Append-only, like the C enum.
...
struct CredentialInput {
    ...
    // guarantee about the process image -- the same claim omegacat_vault_lock
...
public:
    ...
    // For omegacat_capture_open. Zero when not open, which is also the value
    // that call takes to mean "no vault", so an unopened vault degrades to a
    // capture refused for lacking credentials rather than to a bad reference.
    ...
    // "OMEGACAT_VAULT_ERR_LOCKED". For logs, not for users.
