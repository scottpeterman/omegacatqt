...
CredentialDialog::CredentialDialog(Vault *vault, QWidget *parent) : QDialog(parent), m_vault(vault) {
    ...
    // No SNMP types: a capture is SSH only, and the vault surface refuses to
    // store an SNMP credential here. One already in a shared omegamaps vault
    // still lists; it is just never offered to a device.
    ...
    m_tags->setToolTip(tr("A capture can be limited to credentials carrying given tags"));
...
void CredentialDialog::attempt() {
    ...
    // What the vault would accept but a capture could not use.
    QString missing;
