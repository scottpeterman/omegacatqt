 ...
 * cover the static-library case, which is how omegacat links it: a pragma
 * inside an object the linker never pulled in was never read. For that, the
 * top-level CMakeLists.txt puts /INCLUDE:omegacat_go_runtime_init on the link
 * line of everything consuming omegacat::c.
 */
...
static int __cdecl omegacat_start_go_runtime(void) {
...
int(__cdecl *omegacat_go_runtime_init)(void) = omegacat_start_go_runtime;

#pragma comment(linker, "/include:omegacat_go_runtime_init")
