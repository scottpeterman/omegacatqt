// cmd/ocvault/main.go
//
// ocvault — create and manage the credential vault that `capture -vault` reads.
//
...
//
// Example — the two credentials a capture usually needs:
//
//	ocvault init
//	ocvault add -name lab-key -user admin -key ~/.ssh/id_ed25519 -tag lab -priority 10
//	ocvault add -name lab-pw  -user admin -tag lab -priority 20
//	ocvault list
//
// Priority orders the ladder, lower first — so the key is tried before the
// password above. Both carry the "lab" tag, which is what
// `capture -cred-tag lab` selects on.
//
// OmegaCat captures over SSH only, so add refuses SNMP credentials. The file
// format is omegamaps' and a vault holding SNMP entries opens fine — list,
// rm, disable and the rest work on them — they are just never offered.
package main
...
const usage = `ocvault — manage the omegacat credential vault

usage: ocvault [-vault PATH] <command> [flags]
...
run "ocvault <command> -h" for command flags
`
...
func main() {
	...
	flag.Parse()
	if *showVersion {
		fmt.Println(buildinfo.Line("ocvault"))
		return
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "ocvault: unknown command %q\n\n", args[0])
		flag.Usage()
		os.Exit(2)
	}

	if err != nil {
		fmt.Fprintf(os.Stderr, "ocvault: %v\n", err)
		os.Exit(1)
	}
}
...
// cmdKeyring manages the OS-keyring entry that lets a capture unlock the vault
...
func cmdAdd(path string, argv []string) error {
	...
	user := fs.String("user", "", "username: the SSH login (required)")
	keyPath := fs.String("key", "", "private key path; implies -auth publickey")
	authType := fs.String("auth", "", "auth type: password, publickey, keyboard-interactive, agent (default: publickey if -key is set, else password)")
	...
	switch vault.StringToAuthType(at) {
	case vault.AuthSNMPv2c, vault.AuthSNMPv3:
		return fmt.Errorf("%s: omegacat captures over SSH and never offers an SNMP credential; add it with omegamaps' omvault instead", at)
	}
	if *name == "" || *user == "" {
...
func cmdDefault(path string, argv []string) error {
	...
	rest := fs.Args()
	if *clear && len(rest) > 0 {
		return fmt.Errorf("usage: ocvault default [NAME|ID] | ocvault default -clear")
	}
	if len(rest) > 1 {
		return fmt.Errorf("usage: ocvault default [NAME|ID] | ocvault default -clear")
	}
...
func cmdRemove(path string, argv []string) error {
	if len(argv) != 1 {
		return fmt.Errorf("usage: ocvault rm NAME|ID")
	}
	v, err := vaultcli.Open(path)
	if err != nil {
		return err
...
func cmdSetDisabled(path string, argv []string, disabled bool) error {
	if len(argv) != 1 {
		return fmt.Errorf("usage: ocvault disable|enable NAME|ID")
	}
	v, err := vaultcli.Open(path)
	if err != nil {
		return err
