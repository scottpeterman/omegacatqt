 ...
 * omegacat/omegacat.h -- cgo emits a header too, and it is not this one.
 *
 * Ported from omegamaps' vault.h, itself ported from omegassh's: same calls,
 * same codes, and the vault file format is shared, so a vault written by
 * ocvault, omvault, Omega or Pathfinder opens here. omegamaps' vaults can hold
 * SNMP credentials; they list and edit here, but omegacat_vault_store refuses
 * to add or replace one (OMEGACAT_VAULT_ERR_BAD_ARGUMENT), because a capture
 * is SSH only and never offers them.
 *
 ...
 * A capture resolves its credentials inside Go from the vault handle passed to
 * omegacat_capture_open, which is what keeps plaintext -- passwords, community
 ...
 * omegacat_vault_create, omegacat_vault_unlock, omegacat_vault_change_master,
 ...
 *
 * That is the difference from the run API. A capture is minutes long and needs
 * a notifier; this does not. There is no fd to watch here and no callback to
 * marshal.
 *
 * A vault handle passed to omegacat_capture_open is read by the capture for as
 * long as it runs. Locking or closing it mid-capture leaves the devices not yet
 ...
 * caller, who releases it with omegacat_free -- the same deallocator the
...
#ifndef OMEGACAT_VAULT_H
#define OMEGACAT_VAULT_H

/* omegacat_free and omegacat_last_error are shared with the run surface;
 ...
 * Errors
 *
 * The run surface reports failure as -1 plus omegacat_last_error(). That
 ...
 * OMEGACAT_VAULT_ERR_KEYRING_*: everything in it means "the vault could not be
...
typedef enum {
    ...
    OMEGACAT_VAULT_ERR_BAD_HANDLE           = 1,  /* closed, or never opened */
    OMEGACAT_VAULT_ERR_BAD_ARGUMENT         = 2,  /* null pointer, empty path, bad JSON */

    /* Vault state. */
    OMEGACAT_VAULT_ERR_LOCKED               = 10, /* unlock first */
    OMEGACAT_VAULT_ERR_NOT_FOUND            = 11, /* no vault file at that path */
    OMEGACAT_VAULT_ERR_EXISTS               = 12, /* create over an existing one */
    OMEGACAT_VAULT_ERR_WRONG_PASSWORD       = 13, /* GCM auth failure on unlock */
    OMEGACAT_VAULT_ERR_CORRUPT              = 14, /* envelope unparseable or wrong version */
    OMEGACAT_VAULT_ERR_IO                   = 15, /* read/write/rename failed */
    OMEGACAT_VAULT_ERR_WEAK_PASSWORD        = 16, /* below the minimum length on create */

    /* Credential records. */
    OMEGACAT_VAULT_ERR_CRED_NOT_FOUND       = 20,
    OMEGACAT_VAULT_ERR_DUPLICATE_NAME       = 21,
    OMEGACAT_VAULT_ERR_EMPTY_NAME           = 22,
    OMEGACAT_VAULT_ERR_SNMP_DEFAULT       = 23, /* an SNMP credential cannot be the default */
     ...
     * field of omegacat_vault_keyring_status to ask that question. The number
     * stays allocated because this enum is append-only. */
    OMEGACAT_VAULT_ERR_KEYRING_NO_ENTRY     = 30,
    OMEGACAT_VAULT_ERR_KEYRING_UNAVAILABLE  = 31, /* could not be reached at all */
    OMEGACAT_VAULT_ERR_KEYRING_STALE        = 32, /* filed, but no longer unlocks */
    OMEGACAT_VAULT_ERR_NEEDS_PASSWORD       = 33, /* quiet unlock exhausted its sources */

    OMEGACAT_VAULT_ERR_INTERNAL             = 99
} omegacat_vault_err;

/* A short stable identifier for a code ("OMEGACAT_VAULT_ERR_LOCKED"), for
 * logs. Owned by the library, never null, and NOT freed by the caller -- it is
 * the one exception to the omegacat_free rule, because it is a constant. */
const char *omegacat_vault_error_name(omegacat_vault_err code);

/* Detail text for the last failure comes from omegacat_last_error(), shared
 * with the run surface. Free it with omegacat_free.
 *
 ...
 *
 * It is the reason OMEGACAT_VAULT_ERR_KEYRING_UNAVAILABLE is one code rather
 ...
 * omegacat_last_error(). */
long long omegacat_vault_open(const char *path);

/* Release the handle, locking it first. Safe on any value. */
void omegacat_vault_close(long long v);

/* 1 if a vault file is present, 0 otherwise or on a bad handle. */
int omegacat_vault_exists(long long v);

/* 1 if locked. A bad handle reports locked, which is the safe answer. */
int omegacat_vault_is_locked(long long v);

/* The path the handle is bound to. Caller frees with omegacat_free. */
omegacat_vault_err omegacat_vault_path(long long v, char **path_out);
 ...
 * Runs Argon2id. */
omegacat_vault_err omegacat_vault_create(long long v, const char *master);
...
omegacat_vault_err omegacat_vault_unlock(long long v, const char *master);
 ...
 * dialog and call omegacat_vault_unlock.
 *
 ...
 *
 * The environment fallback is OMEGACAT_VAULT_PASSWORD, and it must be present
 ...
 * programmatically, use omegacat_vault_unlock. */
omegacat_vault_err omegacat_vault_unlock_quiet(long long v);
...
void omegacat_vault_lock(long long v);
 ...
 * auto-unlock -- follow a success with omegacat_vault_keyring_set. */
omegacat_vault_err omegacat_vault_change_master(long long v,
 ...
 *   "snmp-v2c", "snmp-v3"    refused by store here (listed for records an
 *                            omegamaps vault already holds)
 *
...
omegacat_vault_err omegacat_vault_list(long long v, char **json_out);
 ...
 * credential a capture will offer. It does not hand over the secret, and there
 * is no variant that does. */
omegacat_vault_err omegacat_vault_meta(long long v, const char *id_or_name,
                                       char **json_out);
 ...
 * omegacat_vault_meta carries no password, so sending it back with an edited
 * name wipes the password. Use omegacat_vault_rename and
 * omegacat_vault_set_disabled for the field-at-a-time cases -- they do the
...
omegacat_vault_err omegacat_vault_store(long long v, const char *json_in,
                                        char **id_out);
 ...
 * not. A form populated from omegacat_vault_meta holds no password, no key
 ...
 * omegacat_vault_clear_default for that.
 *
...
omegacat_vault_err omegacat_vault_update_meta(long long v, const char *json_in);

/* Remove by id. */
omegacat_vault_err omegacat_vault_delete(long long v, const char *id);

/* Rename by id, preserving everything else including the secret. */
omegacat_vault_err omegacat_vault_rename(long long v, const char *id,
                                         const char *new_name);

/* Mark one credential the default, clearing any other. */
omegacat_vault_err omegacat_vault_set_default(long long v, const char *id);

/* Leave no credential marked default. Not an error when none was. */
omegacat_vault_err omegacat_vault_clear_default(long long v);
...
omegacat_vault_err omegacat_vault_default_name(long long v, char **name_out);
...
omegacat_vault_err omegacat_vault_set_disabled(long long v, const char *id,
                                               int disabled);
...
omegacat_vault_err omegacat_vault_keyring_status(long long v, char **json_out);
...
omegacat_vault_err omegacat_vault_keyring_set(long long v, const char *master);

/* Remove the entry. Not an error when there was none. */
omegacat_vault_err omegacat_vault_keyring_clear(long long v);

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* OMEGACAT_VAULT_H */   