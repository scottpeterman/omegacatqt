...
#ifdef _WIN32
#include <windows.h>
static unsigned long long omegacat_thread_id(void) {
...
func setErr(format string, a ...interface{}) {
	errDetail.Store(uint64(C.omegacat_thread_id()), fmt.Sprintf(format, a...))
}

// clearErr drops the message for this thread, so a later successful call does
// not leave an older failure readable.
func clearErr() {
	errDetail.Delete(uint64(C.omegacat_thread_id()))
}

//export omegacat_last_error
func omegacat_last_error() *C.char {
	if v, ok := errDetail.Load(uint64(C.omegacat_thread_id())); ok {
