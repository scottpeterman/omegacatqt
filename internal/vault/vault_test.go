...
// Create makes the vault's directory. The command-line tool always did this
// before calling Create; the application's first run did not, and failed with
// "create temp in ~/.omegacat: no such file or directory" on a machine that
// had never run ocvault. The directory is created owner-only, like the file.
func TestCreateMakesTheVaultDirectory(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, "fresh", ".omegacat", "vault.json")
	v := New(path)
	if err := v.Create("correct horse battery staple"); err != nil {
		t.Fatalf("Create in a directory that does not exist yet: %v", err)
	}
	if !v.Exists() {
		t.Fatal("no vault file after Create")
	}
	if runtime.GOOS != "windows" {
		...
		if perm := st.Mode().Perm(); perm&0o077 != 0 {
			t.Errorf("vault directory mode %o, want owner-only", perm)
		}
	}
}
