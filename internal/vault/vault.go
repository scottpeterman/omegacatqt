...
func (v *Vault) Create(master string) error {
	...
	// The directory too, owner-only. Every front end creates a vault through
	// here, and only the command-line tool used to make the directory first:
	// the application's first run on a machine that had never run ocvault
	// failed writing into a ~/.omegacat that did not exist.
