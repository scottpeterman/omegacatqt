...
// scheduled capture on a headless box can run at all.
const MasterEnvVar = "OMEGACAT_VAULT_PASSWORD"
...
// using a stored entry, clear it (`ocvault keyring clear`) or set
...
func OpenQuiet(path string) (*vault.Vault, error) {
	...
	if err != nil && src == SourceKeyring && errors.Is(err, vault.ErrWrongPassword) {
		...
		return nil, fmt.Errorf("%w: the stored keyring entry no longer unlocks %s "+
			"(re-file it with `ocvault -vault %s keyring set`)", ErrNeedsPassword, path, path)
	}
	return v, err
}
...
// redirected. On a long capture stderr goes to a log file or is discarded, and
	...
	if err != nil && line == "" {
		if errors.Is(err, io.EOF) {
			return "", fmt.Errorf("no %s on stdin and no terminal to prompt on; "+
				"pipe it in, set %s, or store one with `ocvault keyring set`",
				label, MasterEnvVar)
		}
		return "", fmt.Errorf("read %s: %w", label, err)
	}
	...
	if src == SourceKeyring && errors.Is(err, vault.ErrWrongPassword) {
		fmt.Fprintf(os.Stderr,
			"the stored keyring entry did not unlock %s; "+
				"re-file it with `ocvault -vault %s keyring set`\n", path, path)
...
// appHomeDir is omegacat's configuration directory under the user's home.
//
...
// AppDir is ~/.omegacat, or "" when there is no home directory to put it in.
