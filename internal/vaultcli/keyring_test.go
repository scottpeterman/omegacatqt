...
// DefaultPath is ~/.omegacat/vault.json, and a vault sitting in any sibling
// tool's directory must not be adopted in its place: Pathfinder's
// is read by builds that treat SNMP credentials as SSH passwords, and
// omegamaps' is opened on purpose with -vault, never by default.
func TestDefaultPathIsOmegacatAndNeverAdoptsASibling(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home) // windows

	want := filepath.Join(home, ".omegacat", "vault.json")
	for _, dir := range []string{".pathfinderssh", ".pathfinder", ".omegamaps"} {
