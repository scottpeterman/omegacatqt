...
func (s *Session) expect(ctx context.Context, timeout time.Duration) (string, error) {
	...
	for {
		s.mu.Lock()
		tail := promptTail(s.buf)
		readErr := s.err
		over, count := s.overflow, s.received
		if endsAtPrompt(tail, s.prompt) {
			s.lastPrompt = lastLine(tail)
			raw := string(s.buf)
			s.buf = s.buf[:0]
			s.mu.Unlock()
			if over {
			...
			// The one full pass, outside the lock: the reader goroutines
			// are not held up behind a multi-megabyte normalize.
		...
		if looksLikeContinuePrompt(text) {
			if err := s.send(" "); err != nil {
				return s.normalizedBuf(), fmt.Errorf("acknowledging continue prompt: %w", err)
			}
		}
		...
		if readErr != nil {
			...
			return s.normalizedBuf(), fmt.Errorf("session closed while waiting for prompt: %w", readErr)
		}
		select {
		case <-s.notify:
		case <-ctx.Done():
			return s.normalizedBuf(), fmt.Errorf("cancelled waiting for prompt: %w", ctx.Err())
		case <-deadline.C:
			if over {
				return "", s.tooLarge(count, fmt.Sprintf("no prompt within %s", timeout))
			}
			return s.normalizedBuf(), fmt.Errorf("timeout (%s) waiting for prompt; last output: %q",
				timeout, lastLine(text))
		}
	}
}

// promptWindow is how far back from the end of the raw buffer promptTail
// starts looking for a line boundary. A prompt or a pager gate is one short
// line; this only has to be comfortably longer than that. A var so tests can
// shrink it and force the widening path.
var promptWindow = 4 << 10

// promptTail returns the normalized text of the buffer's last few lines —
// enough for lastLine to give the same answer it would give on the whole
// buffer normalized.
//
// This is what keeps expect linear. Normalizing the entire buffer on every
// read notification made a running-config quadratic in its own size: 4 MiB
// arriving in 4 KB reads was a thousand passes over up to 4 MiB each, about
// eleven seconds of CPU with the session mutex held, so the reader goroutines
// stalled behind it and the SSH window backed up onto the device.
//
// Why a cut at a raw newline is safe: every rewrite Normalize makes is
// confined to one line. CSI sequences and the cursor-to-column-one form
// cannot contain a newline, a CR run is only collapsed up to the newline that
// ends it, and CR overwrite works line by line. So for a cut just after a
// '\n', Normalize(buf) == Normalize(buf[:cut]) + Normalize(buf[cut:]), and the
// last line of the whole is the last line of the tail — PROVIDED the tail
// normalizes to something non-blank. It may not: a raw last line of nothing
// but escapes ("prompt#\n\x1b[K") normalizes to empty, and lastLine on the
// whole buffer would then skip back to "prompt#". So an empty answer widens
// the window and tries again rather than being returned; missing a prompt
// here is a 60-second timeout on a healthy device.
//
// The one construct that can span a newline is an OSC string (\x1b] ... BEL).
// Gear uses OSC to set a window title, which carries no newline; an OSC that
// did would at worst make this read one line further back than needed or
// not match until the next read, never match a prompt that is not there.
func promptTail(buf []byte) string {
	window := promptWindow
	for {
		start := len(buf) - window
		if start <= 0 {
			return Normalize(string(buf))
		}
		if i := bytes.IndexByte(buf[start:], '\n'); i >= 0 {
			text := Normalize(string(buf[start+i+1:]))
			if lastLine(text) != "" {
				return text
			}
		}
		// No boundary in the window (one very long line), or the tail was
		// blank once normalized: look further back.
		window *= 2
	}
}

// normalizedBuf is the whole buffer normalized, for the error paths that hand
// back everything received as evidence. Copied under the lock, normalized
// outside it.
func (s *Session) normalizedBuf() string {
	s.mu.Lock()
	raw := string(s.buf)
	s.mu.Unlock()
