...
type Platform struct {
	...
	// Forced is set by FingerprintAs: Name was given, not detected.
	Forced bool
	// Detected is, on a forced platform, what the version output classifies
	// as within the given platform's own family ("" when it matches none of
	// them). It is a second opinion for a caller to report, never a
	// correction: Name stays what was given.
	Detected string
	// PagingRejected is, on a forced platform, that the device answered the
	// platform's paging-disable command with an error. Long output may page,
	// and the given platform is probably wrong.
	PagingRejected bool
}
...
// Platforms lists every platform name Fingerprint can return and
// FingerprintAs accepts, in probe order. "unknown" is not one of them.
...
// KnownPlatform reports whether name is one of Platforms.
...
// FingerprintAs prepares a session for a platform somebody named, without
// classifying the device.
//
// It is not a skipped fingerprint. Fingerprint does two jobs -- naming the
// platform and disabling its paging -- and only the first one is optional:
// a session that never received "terminal length 0" answers the first long
// command with --More-- and waits, and the capture hangs until its timeout.
// So this sends the named platform's own paging command and version command,
// exactly what Fingerprint would have sent had it guessed right, and skips
// only the probes of every other family.
//
// The version output is still classified within that family and reported as
// Detected, and a rejected paging command as PagingRejected. Neither changes
// Name: the caller was told the platform by someone entitled to say.
func FingerprintAs(ctx context.Context, s *Session, name string) (*Platform, error) {
	p, ok := probeFor(name)
	if !ok {
		return nil, fmt.Errorf("platform %q is not one this build knows", name)
	}
	out := &Platform{Name: name, Forced: true, VersionCommand: p.versionCmd}
	if p.paging != "" {
		resp, err := s.Run(ctx, p.paging)
		if err != nil {
			return nil, fmt.Errorf("paging disable %q: %w", p.paging, err)
		}
		if isCLIError(resp) && (p.pagingBenign == nil || !p.pagingBenign.MatchString(resp)) {
			out.PagingRejected = true
		} else {
			out.PagingDisable = p.paging
		}
	}
	ver, err := s.Run(ctx, p.versionCmd)
	if err != nil {
		return nil, fmt.Errorf("version %q: %w", p.versionCmd, err)
	}
	if !isCLIError(ver) {
		out.VersionOutput = ver
		out.Detected = classify(ver, p.classes)
	}
	return out, nil
}
