...
func TestIdentityUsesTheSharedRule(t *testing.T) {
	r := stubResolver{
		ptr: map[string][]string{
			"100.64.4.9": {"spine1.lab.example.net."},
			"100.64.5.9": {"stale.lab.example.net."},
		},
		forward: map[string][]string{"spine1.lab.example.net": {"100.64.4.9"}},
	}
	...
	tests := []struct {
		...
		{"confirmed PTR keys on the name", "100.64.4.9", "spine1"},
		{"stale PTR keys on the address", "100.64.5.9", "100.64.5.9"},
		{"no PTR keys on the address", "100.64.9.9", "100.64.9.9"},
	...
	if Identity("100.64.4.9", suffixes) != Identity("spine1", suffixes) {
...
func TestWalkEmitsCredentialOutcomesWithTheirReason(t *testing.T) {
	type outcome struct{ kind, identity, cred, reason string }
	var got []outcome
	obs := Observer{
		AuthOK: func(identity, cred, reason string) {
			got = append(got, outcome{"ok", identity, cred, reason})
		},
		AuthReject: func(identity, cred, reason string) {
			got = append(got, outcome{"reject", identity, cred, reason})
		},
	}
	store := fakeStore{creds: []vault.Credential{
	...
	r := New(store, NewMemoryBindings(), Config{Observer: obs})
	...
	tried := 0
	won, err := r.Walk(target, func(c vault.Credential) error {
		tried++
		if tried == 1 {
	...
	if len(got) != 2 {
		t.Fatalf("got %d outcomes %+v, want one rejection then one success", len(got), got)
	}
	if got[0].kind != "reject" || got[1].kind != "ok" {
		t.Fatalf("outcome order = %+v, want reject then ok", got)
	}
	for _, o := range got {
		if o.identity != "lab-r1" {
			t.Errorf("%s filed under %q, want lab-r1", o.kind, o.identity)
		}
		if o.cred == "" {
			t.Errorf("%s names no credential", o.kind)
		}
		if o.reason == "" {
			t.Errorf("%s carries no reason; the Try count is uninterpretable without it", o.kind)
		}
	}
	if got[1].cred != won.Name {
		t.Errorf("success names %q, Walk returned %q", got[1].cred, won.Name)
	}
}
