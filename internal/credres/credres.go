...
type Config struct {
	...
	//
	// OmegaCat captures over SSH only and never sets this. It stays because
	// the vault file is shared with omegamaps, where SNMP credentials are
	// real, and an SSH resolver that offered one would spend an attempt
	// (and a lockout count) sending a community string as a password.
	Method Method
	...
	// dial.BaseConfig already learned when it dropped its run-model emitter for
...
// Method is the kind of credential a resolver offers. The values match the
// strings omegamaps' crawl uses, so a binding store written by either tool
// reads the same way.
type Method string

const (
	// MethodSSH offers SSH credentials: password, publickey, agent,
	// keyboard-interactive. The zero value means the same thing.
	MethodSSH Method = "ssh"

	// MethodSNMP offers SNMP credentials only.
...
func (r *Resolver) Resolve(target Target) ([]Candidate, error) {
	...
	for _, c := range all {
		...
		if c.IsSNMP() != (r.cfg.Method == MethodSNMP) {
			continue
		}
