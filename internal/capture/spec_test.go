...
var readOnly = map[string]bool{
	...
	"show ethernet-switching table": true,
}
...
// A configuration that outgrows netexec's session default is refused, not
// truncated, so a config command left on the default fails on every run for
// the largest configs in the estate. Every command of both config types
// carries the explicit bound.
func TestConfigCommandsCarryTheConfigBound(t *testing.T) {
	for _, spec := range []Spec{RunningConfig, StartupConfig} {
		for platform, cmd := range spec.Commands {
			if cmd.MaxBytes != ConfigMaxBytes {
				t.Errorf("%s/%s: MaxBytes = %d, want ConfigMaxBytes (%d)",
					spec.Type, platform, cmd.MaxBytes, ConfigMaxBytes)
			}
		}
	}
}
