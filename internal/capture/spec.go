...
type Command struct {
	...
	// TemplateHint names the TextFSM templates this command's output is
	// parsed with, as a tfsm-fire hint: a platform plus a command stem
	// ("arista_eos_show_mac_address"), which admits every template variant
	// for that command and lets the scorer choose between them. Empty means
	// the output is stored and not parsed.
	//
	// A command stem rather than the platform alone, deliberately. A
	// platform hint puts every template of that platform in contention, and
	// a template too loose to reject output that is not its own can win: on
	// Arista's own MAC-table sample, arista_eos alone picks
	// show_ip_interface_brief. A hint naming exactly one template goes too
	// far the other way -- it leaves nothing to score, and a version variant
	// saved beside it is never tried.
	//
	// Terms of two characters or fewer are dropped by the filter, so a hint
	// needs its distinguishing words spelled at three or more ("cisco_iosxr",
	// not "cisco_xr").
	TemplateHint string
}
...
// ConfigMaxBytes bounds a configuration read. netexec's session default is
// 4 MiB, and a configuration can pass that: an edge router carrying
// IRR-generated prefix-lists for its peers is measured in megabytes, and
// Junos set-style output is larger again than the hierarchical form. Past the
// bound netexec refuses the output rather than truncating it, so a config
// that outgrows the default is not stored short — it fails, on every run,
// on exactly the devices whose config history matters most.
//
// Sixteen MiB is the search limit too (storesearch.DefaultMaxFileBytes is
// defined as this), so anything capture will store, search will read. A
// larger bound here would store configs that search then silently skips.
const ConfigMaxBytes = 16 << 20
...
// that changed instead of the brace level it lives under. It is also the
// larger form, which is part of why every config command carries
// ConfigMaxBytes rather than the session default.
var RunningConfig = Spec{
	Type:        "running-config",
	Description: "the active configuration",
	Commands: map[string]Command{
		"cisco_ios":      {Command: "show running-config", MaxBytes: ConfigMaxBytes},
		"cisco_iosxe":    {Command: "show running-config", MaxBytes: ConfigMaxBytes},
		"cisco_nxos":     {Command: "show running-config", MaxBytes: ConfigMaxBytes},
		"arista_eos":     {Command: "show running-config", MaxBytes: ConfigMaxBytes},
		"juniper_junos":  {Command: "show configuration | display set", MaxBytes: ConfigMaxBytes},
		"aruba_cx":       {Command: "show running-config", MaxBytes: ConfigMaxBytes},
		"aruba_procurve": {Command: "show running-config", MaxBytes: ConfigMaxBytes},
		"hp_comware":     {Command: "display current-configuration", MaxBytes: ConfigMaxBytes},
		"extreme_exos":   {Command: "show configuration", MaxBytes: ConfigMaxBytes},
	},
}
...
var StartupConfig = Spec{
	...
	Commands: map[string]Command{
		"cisco_ios":      {Command: "show startup-config", MaxBytes: ConfigMaxBytes},
		"cisco_iosxe":    {Command: "show startup-config", MaxBytes: ConfigMaxBytes},
		"cisco_nxos":     {Command: "show startup-config", MaxBytes: ConfigMaxBytes},
		"arista_eos":     {Command: "show startup-config", MaxBytes: ConfigMaxBytes},
		"aruba_procurve": {Command: "show config", MaxBytes: ConfigMaxBytes},
		"hp_comware":     {Command: "display saved-configuration", MaxBytes: ConfigMaxBytes},
...
//
// ARP and MAC tables are the two types parsed at capture time (TemplateHint).
// Aruba CX and EXOS carry no hint because the shipped template database has
// no template for their output; their tables are stored and searchable but not
// parsed until one is written.
//
...
var ARPTable = Spec{
	...
	Keep:        5,
	Commands: map[string]Command{
		"cisco_ios":      {Command: "show ip arp", TemplateHint: "cisco_ios_show_arp"},
		"cisco_iosxe":    {Command: "show ip arp", TemplateHint: "cisco_ios_show_arp"},
		"cisco_nxos":     {Command: "show ip arp", TemplateHint: "cisco_nxos_show_arp"},
		"arista_eos":     {Command: "show ip arp", TemplateHint: "arista_eos_show_arp"},
		"juniper_junos":  {Command: "show arp no-resolve", TemplateHint: "juniper_junos_show_arp"},
		"aruba_cx":       {Command: "show arp"},
		"aruba_procurve": {Command: "show arp", TemplateHint: "hp_procurve_show_arp"},
		"hp_comware":     {Command: "display arp", TemplateHint: "hp_comware_display_arp"},
...
//
// Junos takes the full table rather than "brief": both TextFSM templates for
// it (the ntc one and the QFX variant) are written against the full form, and
// a stored table nothing can parse is a table only a literal search can read.
var MACTable = Spec{
	Type:        "mac-table",
	Description: "MAC address to port forwarding entries (rolling history)",
	Keep:        5,
	Commands: map[string]Command{
		"cisco_ios":      {Command: "show mac address-table", TemplateHint: "cisco_ios_show_mac_address_table"},
		"cisco_iosxe":    {Command: "show mac address-table", TemplateHint: "cisco_ios_show_mac_address_table"},
		"cisco_nxos":     {Command: "show mac address-table", TemplateHint: "cisco_nxos_show_mac_address_table"},
		"arista_eos":     {Command: "show mac address-table", TemplateHint: "arista_eos_show_mac_address"},
		"juniper_junos":  {Command: "show ethernet-switching table", ModelMatch: junosSwitch, TemplateHint: "juniper_junos_show_ethernet_switching_table"},
		"aruba_cx":       {Command: "show mac-address-table"},
		"aruba_procurve": {Command: "show mac-address", TemplateHint: "hp_procurve_show_mac_address"},
		"hp_comware":     {Command: "display mac-address", TemplateHint: "hp_comware_display_mac_address"},
