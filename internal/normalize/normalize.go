...
// AddrOf reports whether id is an address, with or without a port, and returns
// the address. "172.16.1.2", "172.16.1.2:2222", "2001:db8::1" and
// "[2001:db8::1]:22" all are; "lab-r1:2222" is not.
//
// The port form matters as much as the bare one. A device reached through a
// port-forward is dialed as 127.0.0.1:2201, and a check that only knows bare
// addresses treats that as a hostname: its "first label" is "127", which every
// other port-forwarded device on the same host also has.
func AddrOf(id string) (netip.Addr, bool) {
	if a, err := netip.ParseAddr(id); err == nil {
		return a, true
	}
	if ap, err := netip.ParseAddrPort(id); err == nil {
		return ap.Addr(), true
	}
