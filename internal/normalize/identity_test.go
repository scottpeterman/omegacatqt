...
func TestResolveWith(t *testing.T) {
	r := stubResolver{
		...
		forward: map[string][]string{
			"spine1.lab.example.net": {"100.64.4.9"},
		},
	}
...
func TestCanonicalWith(t *testing.T) {
	r := stubResolver{
		ptr:     map[string][]string{"100.64.4.9": {"spine1.lab.example.net."}},
	...
	tests := []struct {
		...
		{"CGNAT resolves then strips", "100.64.4.9", suffixes, "spine1"},
		{"CGNAT resolves, no suffixes configured", "100.64.4.9", nil, "spine1.lab.example.net"},
		{"unresolvable CGNAT keys on the address", "100.64.9.9", suffixes, "100.64.9.9"},
...
func TestCanonicalAgreesAcrossForms(t *testing.T) {
	...
	forms := []string{
		"100.64.4.9",              // reached by CGNAT address on one hop
