...
type Target struct {
	...
	// Legacy enables SHA-1 key exchange and CBC ciphers for this device
	// alone. It adds to BaseConfig.Legacy and never subtracts: a run with
	// legacy on is legacy for everyone, and a device that needs it gets it
	// in a run that does not.
	Legacy bool
...
// ApplyTarget is ApplyFor with the per-target settings a Target carries
// layered on: today, Legacy. Every dialer here builds its config through this,
// so a per-device setting cannot be honored by one and missed by another.
func (b BaseConfig) ApplyTarget(t Target) sshcore.Config {
	cfg := b.ApplyFor(t.Target, t.Identity)
	if t.Legacy {
