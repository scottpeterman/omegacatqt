...
// A device that needs legacy algorithms gets them in a run that does not, on
// every attempt of the ladder; one that does not need them still gets them in
// a run that does. Per-device legacy adds to the run's, never subtracts.
func TestPerTargetLegacyAddsToTheRunSetting(t *testing.T) {
	store := fakeStore{
		{ID: "id-a", Name: "wrong", Username: "admin", AuthType: "password", Password: "nope", Priority: 1},
		{ID: "id-b", Name: "right", Username: "admin", AuthType: "password", Password: "correct", Priority: 2},
	}
	for _, tc := range []struct {
		...
		{"run off, device on", false, true, true},
		{"run on, device off", true, false, true},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			d := &Vault{
				res:  credres.New(store, nil, credres.Config{}),
				base: BaseConfig{Legacy: tc.runLegacy},
				dial: func(cfg sshcore.Config) (*sshcore.Client, error) {
					tried = append(tried, cfg)
					if cfg.Password != "correct" {
						return nil, authRejected(cfg.Host)
			...
			if _, err := d.Dial(context.Background(), Target{Target: "172.16.1.2", Identity: "old-sw", Legacy: tc.targLegacy}); err != nil {
				t.Fatalf("dial: %v", err)
			}
			if len(tried) != 2 {
				t.Fatalf("%d attempts, want 2", len(tried))
			}
			for i, cfg := range tried {
				if cfg.LegacyAlgorithms != tc.want {
					t.Errorf("attempt %d: LegacyAlgorithms = %v, want %v", i+1, cfg.LegacyAlgorithms, tc.want)
				}
			}
		})
	}

	// Static takes the same path.
	cfg := BaseConfig{}.ApplyTarget(Target{Target: "172.16.1.2", Legacy: true})
	if !cfg.LegacyAlgorithms {
		t.Error("ApplyTarget dropped the target's Legacy")
	}
}
