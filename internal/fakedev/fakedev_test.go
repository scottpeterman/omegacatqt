...
// The pager fixture itself: long output waits at --More-- until the paging
// command has been sent, and returns normally after it.
func TestPagerWedgesLongOutputUntilPagingIsDisabled(t *testing.T) {
	cfg := fakedev.IOS("lab-r1")
	cfg.Pager, cfg.PageLines = "terminal length 0", 3
	cfg.Commands["show running-config"] = "line 1\nline 2\nline 3\nline 4\nline 5\n"

	_, paging := open(t, cfg, netexec.Options{CommandTimeout: 300 * time.Millisecond})
	if _, err := paging.Run(context.Background(), "show running-config"); err == nil {
		t.Fatal("long output returned a prompt with paging still on")
	}

	_, clean := open(t, cfg, netexec.Options{CommandTimeout: 5 * time.Second, PagingDisable: "terminal length 0"})
	out, err := clean.Run(context.Background(), "show running-config")
	if err != nil || !strings.Contains(out, "line 5") {
		t.Fatalf("after paging disable: %q, %v", out, err)
	}
}
