...
type Config struct {
	...
	// Pager makes the device page like real gear until this command is
	// received in the session: an answer longer than PageLines stops after
	// PageLines lines at " --More-- " and waits for a keypress that a
	// capture never sends -- so the command never returns a prompt. Empty
	// never pages. This is the hang a session that skipped its platform's
	// paging-disable command meets, reproduced rather than assumed.
	Pager string
	// PageLines is the page length with Pager set. Zero is 24.
	PageLines int
...
// should stop driving the loop — a hang, or a page waiting for a keypress,
// where the point is that no prompt ever follows. paged is this shell's
// pager state, cleared by the Pager command.
func (s *Server) answer(ch ssh.Channel, cmd string, paged *bool) bool {
	if cmd == "" {
		io.WriteString(ch, s.cfg.Prompt)
		return true
	}
	...
	if s.cfg.Pager != "" && cmd == s.cfg.Pager {
		*paged = false
	}
	...
	if *paged {
		limit := s.cfg.PageLines
		if limit <= 0 {
			limit = 24
		}
		if lines := strings.Split(strings.TrimRight(out, "\n"), "\n"); len(lines) > limit {
			s.write(ch, strings.Join(lines[:limit], "\n"))
			io.WriteString(ch, " --More-- ")
			return false
		}
	}
	if out != "" {
